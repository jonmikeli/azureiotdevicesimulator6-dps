...
namespace IoT.Simulator.Services
{
    ...
    public class CustomTestResultTelemetryMessageService : ITelemetryMessageService
    {
        private ILogger _logger;
        private string fileTemplatePath = @"./Messages/testResult.v1.json";

        public CustomTestResultTelemetryMessageService(ILoggerFactory loggerFactory)
        {
            if (loggerFactory == null)
                throw new ArgumentNullException(nameof(loggerFactory));

            _logger = loggerFactory.CreateLogger<CustomTestResultTelemetryMessageService>();
        }
        ...
        public async Task<string> GetMessageAsync(string deviceId, string moduleId)
        {
            ...
            string logPrefix = "CustomTestResultTelemetryMessageService".BuildLogPrefix();
            string messageString = await GetMessageAsync();

            if (string.IsNullOrEmpty(messageString))
                throw new ArgumentNullException(nameof(messageString), "DATA: The message to send is empty or not found.");

            _logger.LogTrace($"{logPrefix}::{artifactId}::testResult.v1.json file loaded.");
        ...
        internal string RandomizeCustomData(string jsonMessage)
        {
            ...
            if (data != null)
            {
                ...
                data["result"] = ((double)(r.Next(5000, 200000)/100)).ToString();
                data["sanction"] = r.Next(0, 10) > 5 ? "PASS" : "FAIL";                
