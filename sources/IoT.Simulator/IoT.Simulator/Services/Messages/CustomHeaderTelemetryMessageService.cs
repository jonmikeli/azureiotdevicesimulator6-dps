...
namespace IoT.Simulator.Services
{
    ...
    public class CustomHeaderTelemetryMessageService : ITelemetryMessageService
    {
        private ILogger _logger;
        private string fileTemplatePath = @"./Messages/header.v1.json";

        public CustomHeaderTelemetryMessageService(ILoggerFactory loggerFactory)
        {
            if (loggerFactory == null)
                throw new ArgumentNullException(nameof(loggerFactory));

            _logger = loggerFactory.CreateLogger<CustomHeaderTelemetryMessageService>();
        }
        ...
        public async Task<string> GetMessageAsync(string deviceId, string moduleId)
        {
            ...
            string logPrefix = "CustomHeaderTelemetryMessageService".BuildLogPrefix();
            string messageString = await GetMessageAsync();

            if (string.IsNullOrEmpty(messageString))
                throw new ArgumentNullException(nameof(messageString), "DATA: The message to send is empty or not found.");

            _logger.LogTrace($"{logPrefix}::{artifactId}::header.v1.json file loaded.");
